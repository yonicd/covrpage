...
You can see an example for the `remedy` package [here](https://github.com/ThinkR-open/remedy/tree/master/tests#tests-and-coverage). <!-- When it exists point to the vignette explaining how to read and interpret the report. -->
...
* A vignette, that'll get inserted into the `pkgdown` website of your package. `covrpage::use_covrpage_vignette()` sets it up. Target audience: users reading the rendered documentation.
...
The easiest solution is to run `travis::use_tic()` at the root of your repository, which will ensure your `pkgdown` website is deployed from Travis if you have one. Now, here are the steps to do to add building of the README in tests/. ADD STEPS, probably involving `tic::step_run_code()`.

<!-- GHE stuff to be written by Jonathan. 

Or point to CI vignette? I think it's better if the guidance can be summarized in a few lines here, except for maybe GHE that could go in an "advanced CI settings" vignette (current CI vignette).-->

# Conclusion

`covrpage` report aims at providing more information about the testing of your package to potential users and collaborators, and can inform your own work on your package. Publishing the report with continuous integration will ensure it's always up-to-date. As a summary, in any package of yours, run <!-- having a summary would be nice. -->

```r
covrpage::covrpage()
# usethis::use_pkgdown()
covrpage::use_covrpage_vignette()
travis::use_tic/covrpage::use_covrpage()
```