...
use_covrpage <- function(file = ".travis/covrpage.sh",
    ...
    msg <- whisker::whisker.render(template = readLines(system.file(sprintf('templates/%s/use_covrpage_msg.txt',travis_type),package='covrpage')),
                                   data = list(success=as_render,failure=af_render,deploy_ghpages=deploy_render)
                                   )
  
  if(travis_type=='bash'){
  s_pr <- whisker::whisker.render(template = readLines(system.file(sprintf('templates/%s/use_covrpage.txt',travis_type),package='covrpage')),
